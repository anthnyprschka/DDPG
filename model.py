...
class Actor( object ):
    ...
    def build_actor_network( self, state, trainable ):
        h1 = hidden_layer( state, self.size_h1, name='h1', trainable=trainable )
        h2 = hidden_layer( h1, self.size_h2, name='h2', trainable=trainable )
        h3 = hidden_layer( h2, self.size_h3, name='h3', trainable=trainable )
        out_initializer = tf.random_uniform_initializer( minval=-3*1e-3, maxval=3*1e-3 )
        out = tf.layers.dense( h2, self.a_dim, activation=tf.nn.tanh,
            kernel_initializer=out_initializer, name='out', trainable=trainable )
...
class Critic( object ):
    ...
    def build_critic_network( self, state, action, reuse, trainable ):
        h1s = hidden_layer( state, self.size_h1 , name='c_1', reuse=reuse, trainable=trainable)
        h2s = hidden_layer( h1s, self.size_h2 , name='c_2', reuse=reuse, trainable=trainable)
        h1a = hidden_layer( action, self.size_h2 , name='c_3', reuse=reuse, trainable=trainable)
        h3 = tf.concat( [ h2s, h1a ], axis=1 )
        h3 = hidden_layer( h3, self.size_h3 , name='c_4', reuse=reuse, trainable=trainable)
        q_initializer = tf.random_uniform_initializer( minval=-3*1e-3, maxval=3*1e-3 )
        q = tf.layers.dense( h3, 1, activation=None,
            kernel_initializer=q_initializer, name='c_5', reuse=reuse, trainable=trainable )
        return q
